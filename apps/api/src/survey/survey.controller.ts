// apps/api/src/survey/survey.controller.ts
import { Body, Controller, Get, Param, Post, Res, Query, UseGuards } from '@nestjs/common';
...
export class SurveyController {
  ...
  // Если surveyId не задан -> сводка по всем опросам
  @Get('stats')
  ...
  async exportWide(@Res() res: Response, @Query('surveyId') surveyId?: string) {
    ...
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="answers_wide.csv"');
    return res.send(csv);
  }
  ...
  async exportAllWide(@Res() res: Response) {
    ...
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="answers_all_wide.csv"');
    return res.send(csv);
  }
